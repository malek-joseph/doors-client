...
const Messages: React.FC<MessagesProps> = ({ params }) => {
  ...
  return (
    <div className={`flex flex-col md:flex-row p-4 net_height ${isChatView ? "mt-28 mb-14" : "my-14" }  lg:my-0 md:my-0 `}>
      <div className={`w-full md:w-1/3 h-full border border-gray-300 rounded-lg overflow-y-auto my-3 ${isChatView ? "hidden md:block" : ""}`}>
        {conversations.map((conversation) => (
          <div
            key={conversation._id}
            className={`p-3 cursor-pointer ${selectedConversation?._id === conversation._id ? "bg-gray-200" : ""}`}
            onClick={() => {
            ...
            <div className="flex items-center">
              <div className="w-10 h-10 rounded-full mr-3 bg-gray-300">
                <Image
                  src={conversation.listingOwnerId === userDetails.id ? conversation.currentUserPhoto : conversation.listingOwnerPhoto}
                  alt={conversation.listingOwnerId === userDetails.id ? conversation.currentUserName : conversation.listingOwnerName}
              ...
              <div>
                <div>{conversation.listingOwnerId === userDetails.id ? conversation.currentUserName : conversation.listingOwnerName}</div>
      ...
      <div className={`w-full md:w-2/3 h-full flex flex-col items-center justify-center p-4  ${isChatView ? "" : "hidden md:flex"}`}>
        {isChatView && (
          <>
            <button className="md:hidden mb-4 flex items-center " onClick={() => setIsChatView(false)}>
              <FaArrowLeft className="mr-2" /> Back to Conversations
            </button>
            {selectedConversation && (
              <div className="w-full  max-w-2xl mx-auto border border-gray-300 rounded-lg shadow-md">
                <Chat
